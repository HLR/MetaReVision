...
class NLVR2Dataset:
    """
    An NLVR2 rawdata example in json file:
    {
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open("rawdata/nlvr2/%s.json" % split)))
...
class NLVR2TorchDataset(Dataset):
    def __init__(self, dataset: NLVR2Dataset):
        ...
        if 'train' in dataset.splits:
            img_data.extend(load_obj_tsv('rawdata/nlvr2_imgfeat/train_obj36.tsv', topk=topk))
        if 'valid' in dataset.splits:
            img_data.extend(load_obj_tsv('rawdata/nlvr2_imgfeat/valid_obj36.tsv', topk=topk))
        if 'test' in dataset.name:
            img_data.extend(load_obj_tsv('rawdata/nlvr2_imgfeat/test_obj36.tsv', topk=topk))
        self.imgid2img = {}
