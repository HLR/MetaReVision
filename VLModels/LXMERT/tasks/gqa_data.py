...
# Notice that here is the number of images instead of the number of rawdata,
# which means all related rawdata to the images would be used.
TINY_IMG_NUM = 512
FAST_IMG_NUM = 5000
...
class GQADataset:
    """
    A GQA rawdata example in json file:
    {
        "img_id": "2375429",
        "label": {
            "pipe": 1.0
    ...
    def __init__(self, splits: str):
        ...
        # Loading datasets to rawdata
        self.data = []
        for split in self.splits:
            self.data.extend(json.load(open("rawdata/gqa/%s.json" % split)))
        print("Load %d rawdata from split(s) %s." % (len(self.data), self.name))
        ...
        # Answers
        self.ans2label = json.load(open("rawdata/gqa/trainval_ans2label.json"))
        self.label2ans = json.load(open("rawdata/gqa/trainval_label2ans.json"))
...
class GQABufferLoader():
    ...
    def load_data(self, name, number):
        if name == 'testdev':
            path = "rawdata/vg_gqa_imgfeat/gqa_testdev_obj36.tsv"
        else:
            path = "rawdata/vg_gqa_imgfeat/vg_gqa_obj36.tsv"
...
class GQATorchDataset(Dataset):
    def __init__(self, dataset: GQADataset):
        ...
        img_data = []
        if 'testdev' in dataset.splits or 'testdev_all' in dataset.splits:     # Always loading all the rawdata in testdev
        ...
        # Only kept the rawdata with loaded image features
        self.data = []
        for datum in self.raw_dataset.data:
            if datum['img_id'] in self.imgid2img:
                self.data.append(datum)
        print("Use %d rawdata in torch DataSet" % (len(self.data)))
        print()
