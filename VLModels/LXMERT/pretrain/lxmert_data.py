...
Split2ImgFeatPath = {
    'mscoco_train': 'rawdata/mscoco_imgfeat/train2014_obj36.tsv',
    'mscoco_minival': 'rawdata/mscoco_imgfeat/val2014_obj36.tsv',
    'mscoco_nominival': 'rawdata/mscoco_imgfeat/val2014_obj36.tsv',
    'vgnococo': 'rawdata/vg_gqa_imgfeat/vg_gqa_obj36.tsv',
}
...
class LXMERTDataset:
    def __init__(self, splits: str, qa_sets=None):
        """
        :param splits: The rawdata sources to be loaded
        :param qa_sets: if None, no action
                        o.w., only takes the answers appearing in these dsets
                              and remove all unlabeled rawdata (MSCOCO captions)
        """
        self.name = splits
        ...
        for source in self.sources:
            self.data.extend(json.load(open("rawdata/lxmert/%s.json" % source)))
        print("Load %d rawdata from %s" % (len(self.data), self.name))
...
class LXMERTEvaluator:
    ...
    def evaluate(self, uid2ans: dict, pprint=False):
        ...
        for uid, ans in uid2ans.items():
            if uid not in self.uid2datum:   # Not a labeled rawdata
                continue
