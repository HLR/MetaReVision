...
class AnswerTable:
    ...
    def __init__(self, dsets=None):
        self.all_ans = json.load(open("rawdata/lxmert/all_ans.json"))
...
def load_lxmert_qa(path, model, label2ans):
    ...
    The answers in the fine-tuned QA raw_task (indicated by label2ans)
    ...
    # Load Bert Weights
    bert_model_keys = set(model.lxrt_encoder.lxmert_pretrain_model.state_dict().keys())
    bert_loaded_keys = set(bert_state_dict.keys())
    assert len(bert_model_keys - bert_loaded_keys) == 0
    model.lxrt_encoder.lxmert_pretrain_model.load_state_dict(bert_state_dict, strict=False)
