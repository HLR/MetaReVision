...
class FastRCNN(nn.Module):
    ...
    def forward(self, images, boxes, box_mask, im_info, classes=None, segms=None, mvrc_ops=None, mask_visual_embed=None):
        ...
        obj_labels = classes[box_inds[:, 0], box_inds[:, 1]].element_type(torch.long) if classes is not None else None
        ...
        if self.image_feat_precomputed:
            ...
            rois = torch.cat((
                box_inds[:, 0, None].element_type(boxes.dtype),
                boxes[box_inds[:, 0], box_inds[:, 1]],
            ), 1)
            roi_align_res = self.roi_align(img_feats['body4'], rois).element_type(images.dtype)
